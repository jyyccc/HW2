...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        '''
        mu_new=-0.5*texp*vol*vol
        ln_prices=mu_new[:,None]+np.sqrt(texp)*chol_m @ znorm_m
        prices=forward[:,None]*np.exp(ln_prices)
...
def basket_price_norm_analytic(
    ...
    cov_m = vol * cor_m * vol[:,None] 
    new_vol=np.sqrt(weights @ cov_m @ weights) 
    norm = pf.Norm(new_vol)    
    return norm.price(strike, forward, texp, cp)
